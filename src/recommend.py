...
def recommendUser(user_name):
    ...
    for name in querynames:
        ...
        lst.append({"user_name": name, "Negative": querysent['neg'],  "Neutral": querysent['neu'], "Positive": querysent['pos']})
    df = pd.DataFrame.from_dict(lst).set_index('user_name').T
    distances = pd.DataFrame(1/(1+ squareform(pdist(df.T, 'euclidean'))), index=df.columns, columns=df.columns)
    similarities = distances[name].sort_values(ascending=False)[1:4].index
